...
const Perfumes = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('https://mark1-nf8t.onrender.com/api/clothing-items'); // Replace with your API endpoint for clothing items
        const data = await response.json();

        // Filter items by category 'Perfumes'
        const perfumeItems = data.filter(item => item.category === 'Perfumes');
  ...
  const addToCart = async (item) => {
    const { id, title, imageUrl, price, category } = item; // Destructure needed properties
      ...
      if (existingItem) {
        ...
        await fetch('https://mark1-nf8t.onrender.com/api/cart', {
          ...
          body: JSON.stringify({
            ...
            price: price,
            quantity: 1,
            category: category // Include category in the POST request
          }),
        });
